...
export function buildEarthWorld({scene,world,RAPIER}){
 ...
 const forestGlows=[];for(let i=0;i<24;i++){const side=i%2?-1:1,x=side*(7+random()*19),z=-11-random()*38;const stem=mesh(new THREE.CylinderGeometry(.12,.22,.8+random()*.8,7),material(0xdac9c7),x,.5,z),cap=mesh(new THREE.SphereGeometry(.48+random()*.35,9,6,0,Math.PI*2,0,Math.PI/2),glowMats[i%2],x,1.05,z);cap.scale.y=.55;forestGlows.push(cap);if(i%3===0){const crystal=mesh(new THREE.OctahedronGeometry(.55+random()*.55),glowMats[(i+1)%2],x+side*1.2,.65,z+.8);crystal.scale.y=1.8;forestGlows.push(crystal)}}
 ...
 const highlandRocks=[];for(let i=0;i<30;i++){const side=i%2?-1:1,x=side*(8+random()*21),z=-59-random()*55,r=1+random()*2.3;const rock=mesh(new THREE.DodecahedronGeometry(r,0),material(i%2?0xa66048:0x72586d),x,r*.3,z);rock.scale.set(.8+random()*.7,1.1+random()*1.5,.75+random()*.7);highlandRocks.push(rock);if(i%5===0)world.createCollider(RAPIER.ColliderDesc.ball(r*.7).setTranslation(x,r*.3,z))}
