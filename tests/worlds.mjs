...
assert.ok(world.colliders.len()>=90,'asteroids, ships, creatures, relays and portal rim are solid');assert.match(score.textContent,/אנרגיה 0\/3/);assert.equal(space.asteroids.length,58);assert.equal(space.saucers.length,3);assert.equal(space.monsters.length,3);assert.equal(scene.children.some(o=>o.geometry?.type==='PlaneGeometry'),false,'space contains no ground plane');
...
space.reset();assert.equal(win.hidden,true);assert.ok(world.colliders.len()>=90,'asteroids, ships, creatures, relays and portal rim are solid');assert.match(score.textContent,/אנרגיה 0\/3/);assert.equal(space.asteroids.length,58);assert.equal(space.saucers.length,3);assert.equal(space.monsters.length,3);assert.equal(scene.children.some(o=>o.geometry?.type==='PlaneGeometry'),false,'space contains no ground plane');
const followWorld=new RAPIER.World({x:0,y:0,z:0});followWorld.timestep=1/60;const companion=new Companion(followWorld,new THREE.Group());followWorld.step();
for(let i=0;i<600;i++){companion.step({x:0,y:3,z:12-Math.min(i,360)/30},1/60);followWorld.step()}
...
const movingWall=testWorld.createRigidBody(RAPIER.RigidBodyDesc.kinematicPositionBased().setTranslation(0,3,0));testWorld.createCollider(RAPIER.ColliderDesc.cuboid(5,5,.2),movingWall);
...
for(let i=0;i<180;i++){ctrl.computeColliderMovement(capsule,{x:0,y:0,z:-.1});const p=actor.translation(),d=ctrl.computedMovement();actor.setNextKinematicTranslation({x:p.x+d.x,y:p.y+d.y,z:p.z+d.z});testWorld.step()}
assert.ok(actor.translation().z>.54,'character controller stops at moving kinematic bodies');
...
world.free();followWorld.free();testWorld.free();
